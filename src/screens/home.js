...
export function Home({ navigation }) {
  ...
  const [popUpData, setPopUpData] = useState({ visible: '0', name: '', id: '', mobile: '', country: '', about: '' });
  ...
  const loadFriendList = useCallback(async () => {
    try {
      ...
      // Stabilize updates to prevent flickering
      setItems(prevItems => {
  ...
  useFocusEffect(
    useCallback(() => {
      loadFriendList();
      const interval = setInterval(loadFriendList, 5000);
  ...
  const ItemUI = React.memo(({ item, index }) => (
    <Animatable.View animation={'fadeInUp'} duration={600} delay={index * 50}>
      <TouchableOpacity
        onPress={() => navigation.navigate('Chat', { ...item, img: item.pic })}
        activeOpacity={0.7}
      >
        <ThemedCard style={styles.itemContainer}>
          <TouchableOpacity
            onPress={() => setPopUpData({ visible: item.pic, name: item.name, id: item.id, mobile: item.mobile, country: item.country, about: item.about })}
            activeOpacity={0.9}
          >
            <Image
          ...
          <View style={styles.itemMeta}>
            <ThemedText type="caption" color="textLight">
              {item.time}
            </ThemedText>
            {parseInt(item.count) > 0 && (
              <View style={styles.badge}>
                <ThemedText type="caption" weight="bold" color="textOnPrimary">
                  {item.count}
                </ThemedText>
              </View>
            )}
  ...
  return (
    <SafeAreaView style={styles.home}>
      ...
      <View style={styles.header}>
        <ThemedText type="title" color="primary">Messages</ThemedText>
        <TouchableOpacity onPress={openSettings}>
          <Image
            source={{ uri: HOST_BASE_URL + user.profile_url }}
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('Search', { pageId: '1' })}
        activeOpacity={0.8}
      >
        <Icon name="search-outline" size={20} color={theme.colors.textLight} style={styles.searchIcon} />
        <ThemedText color="textLight">Search conversations...</ThemedText>
      </TouchableOpacity>
      ...
      <ImgPopUp visible={popUpData.visible} onClose={() => setPopUpData({ ...popUpData, visible: '0' })}>
        <ThemedCard style={styles.popUpCard}>
          ...
          <View style={styles.popUpFooter}>
            <TouchableOpacity onPress={() => { navigation.navigate('Chat', { name: popUpData.name, id: popUpData.id, img: popUpData.visible }); setPopUpData({ ...popUpData, visible: '0' }); }} style={styles.imgPopUpBnt}>
              <Icon name="chatbubbles-outline" size={28} color={theme.colors.primary} />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => { navigation.navigate('UserProfile', { img: popUpData.visible, mobile: popUpData.mobile, name: popUpData.name, country: popUpData.country, pageId: '2', id: popUpData.id, about: popUpData.about }); setPopUpData({ ...popUpData, visible: '0' }); }} style={styles.imgPopUpBnt}>
              <Icon name="person-outline" size={28} color={theme.colors.primary} />
            </TouchableOpacity>
          </View>
        </ThemedCard>
      </ImgPopUp>
