...
export function Search({ navigation, route }) {
  ...
  const loadFriendList = useCallback(async (text) => {
    try {
      ...
      <TouchableOpacity
        onPress={() => {
          const obj = {
            ...
            id: item.id,
            img: item.pic,
            imageUrl: route.params?.pageId == '3' ? route.params.img : '',
            pageId: '3',
          };
          ...
          <View style={styles.itemContent}>
            ...
            {(item.chatStatusId == 2 || (item.msg && (item.msg.includes('/upload/') || item.msg.includes('/postUpload/') || item.msg.includes('/groupImg/')))) ? (
              ...
              <ThemedText type="small" color="textSecondary" numberOfLines={1}>
                {item.msg || 'Tap to start chatting'}
              </ThemedText>
            )}
          </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            <Icon name="people-outline" size={60} color={theme.colors.textLight} />
            <ThemedText color="textLight" style={{ marginTop: 10 }}>No users found</ThemedText>
          </View>
        }
      />
    </SafeAreaView>
